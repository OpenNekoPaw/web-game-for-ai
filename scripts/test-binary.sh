...
curl --fail --silent --request POST --header 'content-type: application/json' --data '{}' "$base_url/agent/v1/games" | node -e "let value=''; process.stdin.on('data', chunk => value += chunk).on('end', () => { const data = JSON.parse(value); if (!data.gameId || data.protocol !== 'agent-game.v1') process.exit(1); });"
